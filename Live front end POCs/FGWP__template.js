...
$(function(){
	...
	var fgwp = {
		...
		productImage: "//img.smartpak.com/images/product/highres/24237_outsmart_128oz.jpg",
		offerHead: 'Get a Free Gallon of Outmart',
		offerSubTxt: 'with any new AutoShip supplement purchase',
		offerLinkText: 'Get a Free Gallon of OutSmart*',
		offerDisclaimer: '*Receive a free gallon of OutSmart, item #24237, with the purchase of a new AutoShip SmartPak or new AutoShip bucket. Limit one gallon of OutSmart per Customer. Offer does not apply to shipping charges. Offer applies to in-stock items only. Offer expires 6/25/18 at 11:59 PM PST. Order must ship by 6/27/18. Additional exclusions may apply. Please call 1-800-461-8898 for complete details.',
		qualifiedSubText: "for a free gallon of OutSmart!",
		qualifiedLinkText: "Click Here to add your FREE gallon of OutSmart",
		...
		addFGWPRow: function(){
			...
			$('#fgwpPopUp').remove();
			$("<div id='fgwpPopUp' class='jsHide'>"+smartPak.fgwp.offerDisclaimer+"</div>").appendTo('body');
		},
		...
		updateToProduct: function(){
			$('#fgwpImg').attr('src', smartPak.fgwp.productImage+'?width=80');
			$('#fgwpHd').text(smartPak.fgwp.offerHead);
			$('#subTxt').text(smartPak.fgwp.offerSubTxt  + " (");
			$('#offerAction a').text(smartPak.fgwp.offerLinkText);
			if(this.isMobile){
			...
			$('#fgwpPopUp').text(smartPak.fgwp.offerDisclaimer);
		},	

		updateToAddToCart: function(giftType){
			var btnData = smartPak.fgwp.productData;			


			$('#fgwpHd').text('Your Order Qualifies!');

			if(giftType === 'product'){ 
				$('#subTxt').text(smartPak.fgwp.qualifiedSubText + " (");
				$('#offerAction a').replaceWith('<a href="#" class="addGiftToCart">'+smartPak.fgwp.qualifiedLinkText+'</span></a>');
				$('.addGiftToCart').data(btnData);
			}
		...
		updateFGWPItem: function(){ 
			...
			if(!smartPak.fgwp.hasATSSupplementInCart()) $removeBtn.click();
