$(function(){
	...
	var fgwp = {
		fgwpStyles: '<style> #fgwpImg {width: 80px; float:left; margin-right: 24px; } #fgwpContent > * {display: inline-block; text-align: left; } #fgwpRow {text-align: right; clear: both; margin-top: 64px; } #fgwpHd { text-align: right; font-size: 19px; font-weight: 600; color: #2A51AB; } #fgwpSubTxt {font-size: 15px; font-weight: 500; margin-top: -4px; text-align: center; } #fgwpCopy {vertical-align: top; margin: 8px 50px 0 20px; width:400px; } #fgwpContent {margin-top: 16px; } #offerLabel {display: inline-block; margin-right: 16px; } #offerAction {display: inline-block; } #fgwpOfferCol {vertical-align: bottom; margin-bottom: 6px; font-size: 16px; background: #f3f3f3; padding: 2px 12px; } #fgwpSubTxt a {padding: 0 2px; font-size:12px; } #fgwpPopUp {margin: 30px 20px; font-size: 16px; } .addGiftToCart span {font-size: 13px; margin-left: 4px; }</style>',
		...
		hasFGWPInCart: function(){
			...
			for(i in items){
				if(items[i].ProductClassId){
					if(items[i].ProductClassId() === 18129) return true;
				}
			}
			return false;
		},
...
/*			if(giftType === 'hat'){
		...
		updateFGWPItem: function(){ 
			...
			$('[href="/pt/buy-a-new-sp-or-bucket-on-ats-and-get-a-free-sp-slow-feed-hay-bag-18129"]').on('click', function(e){
				e.preventDefault();
				return false;
			});
